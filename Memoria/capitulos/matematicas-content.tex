...
Sean $C_{i}$ y $C_{j}$ dos clusters cualesquiera con $n_{i}$, $n_{j}$ elementos, respectivamente, y sean $x_{ik} \in C_{i}$ , $x_{jm} \in C_{j}$, con $k=1,\dots,n_{i}$, $m=1,\dots,n_{j}$ dos elementos de dichos clusters. La 
distancia entre $C_{i}$ y $C_{j}$ viene dada por

\[
d(C_{i},C_{j}) = \min_{\substack{k=1,\dots,n_{i} \\ m=1,\dots,n_{j}}}\{d(x_{ik},x_{jm}) \text{ | } x_{ik} \in C_{i} \text{ y } x_{jm} \in C_{j}\}
\]

donde $d(x_{ik},x_{jk})$ es la distancia Euclidea, generalmente, o cualquier otra distancia entre los elementos $x_{ik},x_{jk}$ de los clusters. 

Equivalentemente, en un contexto en el que usemos similaridad, se unirán dos clusters $C_{i}, C_{j}$ si

\[
s(C_{i},C_{j}) = \max_{\substack{k=1,\dots,n_{i} \\ m=1,\dots,n_{j}}}\{s(x_{ik},x_{jm}) \text{ | } x_{ik} \in C_{i} \text{ y } x_{jm} \in C_{j}\}
\]
...
Sean entonces $C_{i}$ y $C_{j}$ dos clusters cualesquiera con $n_{i}$, $n_{j}$ elementos, respectivamente y sean $x_{ik} \in C_{i}$ , $x_{jm} \in C_{j}$, 
...
\[
d(C_{i},C_{j}) = \max_{\substack{k=1,\dots,n_{i} \\ m=1,\dots,n_{j}}}\{d(x_{ik},x_{jm}) \text{ | } x_{ik} \in C_{i} \text{ y } x_{jm} \in C_{j}\}
\]

Equivalentemente, si lo miramos desde el punto de vista de la similaridad, se unirán dos clusters $C_{i}, C_{j}$ si

\[
s(C_{i},C_{j}) = \min_{\substack{k=1,\dots,n_{i} \\ m=1,\dots,n_{j}}}\{s(x_{ik},x_{jm}) \text{ | } x_{ik} \in C_{i} \text{ y } x_{jm} \in C_{j}\}
\]
...
Consideremos en lo que sigue dos clusters $C_{i}$, $C_{j}$ con $n_{i},n_{j}$ elementos, y $x_{ik},x_{jm}$ dos elementos de $C_{i},C_{j}$, respectivamente con $k=1,\dots,n_{i}$, $m=1,\dots,n_{j}$.

\begin{itemize}
    \item \textbf{No ponderada} \newline

    Supongamos, sin pérdida de generalidad, que $C_{i}$ está constituido por dos clusters $C_{i1}$ y $C_{i2}$ con $n_{i1},n_{i2}$ elementos respectivamente, la distancia entre $C_{i}$ y $C_{j}$ viene dada por
    \[
    d(C_{i},C_{j}) = \frac{d(C_{i1}, C_{j}) + d(C_{i2}, C_{j})}{2}
    \]

    Observamos que esta distancia es independiente de los tamaños de los clusters involucrados. Esto quiere decir que las distancias respecto a $C_{j}$ de $C_{i1}$ y $C_{i2}$
    ...
    $C_{i}$ está constituido por dos clusters $C_{i1}$, $C_{i2}$ con $n_{i1}$,$n_{i2}$ elementos tales que $n_{i} = n_{i1}+n_{i2}$ y tomemos los elementos $x_{ik_{1}} \in C_{i1}$, $x_{ik_{2}} \in C_{i2}$. Entonces, 
    la distancia promedio pondera entre $C_{i},C_{j}$ viene dada por
    
    \[
    d(C_{i},C_{j}) = \frac{\sum_{k=1}^{n_{i}}\sum_{m=1}^{n_{j}}d(x_{ik},x_{jm})}{n_{i}n_{j}} = \frac{1}{(n_{i1}+n_{i2})n_{j}} \sum_{k=1}^{n_{i1}+n_{i2}} \sum_{m=1}^{n_{j}} d(x_{ik},x_{jm}) = 
    \]
    \[
    = \frac{1}{(n_{i1}+n_{i2})n_{j}} \sum_{k_{1}=1}^{n_{i1}} \sum_{m=1}^{n_{j}} d(x_{ik_{1}},x_{jm}) + \frac{1}{(n_{i1}+n_{i2})n_{j}} \sum_{k_{2}=1}^{n_{i2}} \sum_{m=1}^{n_{j}} d(x_{ik_{2}},x_{jm}) = 
    \]
    \[
    = \frac{n_{i1}}{(n_{i1}+n_{i2})n_{j}n_{i1}} \sum_{k_{1}=1}^{n_{i1}} \sum_{m=1}^{n_{j}} d(x_{ik_{1}},x_{jm}) + \frac{n_{i2}}{(n_{i1}+n_{i2})n_{j}n_{i2}} \sum_{k_{2}=1}^{n_{i2}} \sum_{m=1}^{n_{j}} d(x_{ik_{2}},x_{jm}) = 
    \]
    \[
    = \frac{n_{i1}}{n_{i1}+n_{i2}} d(C_{i1},C_{j}) + \frac{n_{i2}}{n_{i1}+n_{i2}} d(C_{i2},C_{j}) = 
    \]
    \[
    = \frac{n_{i1}d(C_{i1},j) + n_{i2}d(C_{i2},C_{j})}{n_{i1}+n_{i2}}
    \]
...
\[
\bar{x}_{ik} = \frac{\sum_{k=1}^{n_{i}}x_{ik}}{n_{i}} = \begin{bmatrix} \bar{x}_{i1} \\  \bar{x}_{i2} \\ \vdots \\ \bar{x}_{n_{i}} \end{bmatrix} \text{ y } \bar{x}_{jm} = \frac{\sum_{m=1}^{n_{j}}x_{jm}}{n_{j}} = \begin{bmatrix} \bar{x}_{j1} \\  \bar{x}_{j2} \\ \vdots \\ \bar{x}_{n_{j}} \end{bmatrix} 
\]

respectivamente, y el cuadrado de la distancia Euclidea entre ambos centroides es $d^{2}(C_{i},C_{j}) = \|\bar{x}_{ik} - \bar{x}_{jm}\|^{2}$. Se persigue entonces el objetivo de encontrar clusters que hagan mínima la distancia entre
sus centroides, para, consecuentemente, combinarlos en un nuevo cluster, que llamaremos $C_{t}$ con centroide $\bar{x}_{tl} = \frac{(n_{i}\bar{x}_{ik} + n_{j}\bar{x}_{jm})}{n_{i}+n_{j}}$. Se calcularían de nuevo las distancias entre dicho centroide y el de los demás clusters. Continuaríamos el proceso 
hasta obtener un único cluster. \newline
